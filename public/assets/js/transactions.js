function transactionCSV() {
    ...
    let csv = 'Receipt #,Customer Name ,Date,Items,Quantity,VATable,VAT,Total Amount,Cash Amount,GCASH Amount,Card Amount,Total Payment,Change,Payment Method,Payment,Cashier\n';
    ...
    $('#inventoryTableBody tr').each(function(index, row) {
        ...
        if (!filtersApplied || ((startDate === '' || transactionDateTime >= new Date(startDate)) &&
            ...
            let vatable = $(row).find('td:eq(5) .vatable').text().replace('₱', '').replace(',', '');
            let vat = $(row).find('td:eq(6) .vat').text().replace('₱', '').replace(',', '');
            
            let cashAmount = $(row).find('td:eq(7) .paid_amount').text().replace('₱', '').replace(',', '');
            let gcashAmount = $(row).find('td:eq(8) .paid_amount').text().replace('₱', '').replace(',', '');
            let cardAmount = $(row).find('td:eq(9) .paid_amount').text().replace('₱', '').replace(',', '');
            let totalAmount = $(row).find('td:eq(10) .total_amount').text().replace('₱', '').replace(',', '');
            let totalPayment = $(row).find('td:eq(11) .total_payment').text().replace('₱', '').replace(',', '');
            let change = $(row).find('td:eq(12) .customer-change').text().replace('₱', '').replace(',', '');
            ...
            csv += `"${receipt}","${customerName}","${date}","${itemsStr}","${quantityStr}","${vatable}","${vat}","${totalAmount}","${cashAmount}","${gcashAmount}","${cardAmount}","${totalPayment}","${change}","${paymentMethod}","${payment}","${cashier}"\n`;
        }
    });
